...
while True:
    health = subprocess.run(["docker", "inspect", "--format={{.State.Health.Status}}", "pyspark-notebook"], capture_output=True, text=True)
...
logs = subprocess.run(["docker-compose", "logs", "pyspark-notebook"], capture_output=True, text=True)
